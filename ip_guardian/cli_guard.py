...
for source in "${{IP_SOURCES[@]}}"; do
  candidate_ip=$(curl -fsS --max-time 5 "$source" 2>/dev/null | tr -d '[:space:]')
  if is_valid_ipv4 "$candidate_ip"; then
...
def generate_wrappers(cli_commands, allowed_ips, ip_sources=None):
    """Generate wrapper scripts in ~/.ip_guardian_bin/."""
    write_allowed_ips(allowed_ips)
    effective_sources = resolve_ip_sources(ip_sources)
    ...
    for entry in cli_commands:
        ...
        sources_str = " ".join(f'"{s}"' for s in effective_sources)
        script = WRAPPER_SCRIPT.format(
            cmd=cmd,
            real_path=real_path,
            allowed_subs=subs_str,
            ip_sources=sources_str,
        )
